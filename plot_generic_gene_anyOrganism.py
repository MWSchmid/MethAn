...
usage:
python %s chromSizesFile gffFile stats figure

REQUIRED:
\tchromSizesFile: a file with chromName<tab>chromSize
\tgffFile: a GFF file
...
NOTE:
In contrast to the plotter for arabidopsis, this one takes everything from the GFF with feature == 'gene'!
""" % sys.argv[0]
...
class genomeHandler:
    ...
    def __init__(self, skipOV, chromSizesFile, gffFile):
        self.gffFile = gffFile
        self.chromsizes = {}
        with open(chromSizesFile, 'rb') as infile:
            for line in infile:
                chrom, size = line[:-1].split('\t')
    ...
    def loadAnnotation(self):
        with open(self.gffFile, 'r') as infile:
            for line in infile:
                ...
                strand = fields[6]
                restDict = dict([x.split('=') for x in fields[8].split(';')])
                agi = restDict["ID"]
    ...
    def loadPositions(self, statsFile):
        with open(statsFile, 'r') as infile:
            for line in infile:
                ...
                agis = [x.split(',')[0] for x in agifield.split('|')]
...
### runner
genome = genomeHandler(skipOV, chromSizesFile, gffFile)
