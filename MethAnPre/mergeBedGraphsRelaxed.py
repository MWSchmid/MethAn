...
NOTE: minCov >= 1, maxCov <= 1000, and minGroupCount >= 1 are hard-coded.
...
def mergeFiles(sampleTab, chrom, ctxt, outfileName, fileType="MethAn_bismark", imputation=False):
    ...
    for interval in regions:
        ...
        minGroupCount = 1
        groupCounts = mergedTab.query("coverage>=1 & coverage<=1000").groupby(by=["pos", "group"])["coverage"].size()
        ...
        selectedPositions.query("coverage>=1 & coverage<=1000").to_csv(outfileName, header=writeHeader, sep='\t', index=False, mode='a')
        ...
        mergedTab.query("coverage>=1 & coverage<=1000").to_csv(outfileNameNoGroupFilter, header=writeHeader, sep='\t', index=False, mode='a')
