...
command="trim_galore --dont_gzip --illumina --paired --clip_R2 2 -o ${outputDir} ${inputDir}/${inputFile} ${inputDir}/${inputFileReverse}"
echo "=== ${me}: Running: ${command}"
eval $command
rc=$?
echo "=== ${me}: Command ended with exit code $rc"
remove_if_present ${outputDir}/"${inputFile%%.*}_1_trimmed.fq"
remove_if_present ${outputDir}/"${inputFileReverse%%.*}_2_trimmed.fq"
inputFile="${inputFile%%.*}_val_1.fq"
inputFileReverse="${inputFileReverse%%.*}_val_2.fq"
